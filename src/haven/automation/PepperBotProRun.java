...
public class PepperBotProRun extends Window implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
                    ...
                    if (PBotUtils.invFreeSlots(ui) < 4 && !stopThread) {
                        ...
                        while (PBotUtils.invFreeSlots(ui) < 4 && !stopThread) {
                            ...
                            if (pepperlist.size() == 0) {
                                ...
                                PBotUtils.sleep(1000);
                                ui.gui.map.wdgmsg("click", hfire.sc, hfire.rc.floor(posres), 1, 0, 0, (int) hfire.id, hfire.rc.floor(posres), 0, -1);
                                PBotUtils.sleep(2000);
                                if (section == 2) {
            ...
            PBotUtils.sysMsg(ui, "Section finished!", Color.white);
            ui.gui.act("travel", "hearth");
            PBotUtils.sleep(6000);
            lblProg2.settext("Moving to harvest");
            if (section == 1) {
